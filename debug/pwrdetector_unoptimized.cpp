...
class PWRDetectorUnoptimized : public Detector {
    private:
        struct EpochVCPair {
            Epoch epoch;
        ...
        void debug_print(std::string text, ThreadID thread_id, ResourceName resource_name) {
            ...
            for(auto pairs : current_read_write_events) {
                ...
                printf(" } %s ", pairs.is_write ? "write" : "read");
            }
            printf("}\n");
        ...
        std::vector<EpochLSPair> update_read_write_events(ThreadID thread_id, ResourceName resource_name, bool is_write, std::vector<EpochLSPair> rw_pairs, VectorClock vc, std::vector<ResourceName> ls) {
            // (i#Th(i)[i], LS_t(i))
            // Current "timestamp" of the calling thread with its current locks.
            std::vector<EpochLSPair> read_write_events_new = {
                EpochLSPair { Epoch { thread_id, vc.find(thread_id) }, ls, is_write }
            };
            ...
            for(EpochLSPair rw_pair : rw_pairs) {
                ...
                } else if (!is_write && rw_pair.is_write) {
                    read_write_events_new.push_back(rw_pair);
                }
            }
    ...
    public:
        ...
        void fork_event(ThreadID thread_id, TracePosition trace_position, ThreadID target_thread_id) {
            ...
            vector_clocks.find(target_thread_id)->second.increment(target_thread_id);
