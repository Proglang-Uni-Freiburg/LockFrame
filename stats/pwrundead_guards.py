...
def build_and_write_results(limit):
    ...
    with open(os.path.join(RESULTS_PATH, 'results.csv'), 'a', buffering=1) as results_file:
        results_file.write("trace,vc_limit,races_pwrundead,races_pwrundeadguard,time_taken_pwrundead,time_taken_pwrundeadguard,dependencies_per_thread_pwrundead,dependencies_per_thread_pwrundeadguard,possible_guard_lock_dependencies,possible_guard_locks,guard_locks_accepted,guard_locks_declined\n")
        for logfile in LOG_FILES:
            ...
            results_file.write(re.search(r"^Found (\d+) races.$", result_pwrundeadguard.stdout, flags=re.MULTILINE).group(1) + ",")

            results_file.write(str(timeTaken_pwrundead) + ',')
            results_file.write(str(timeTaken_pwrundeadguard) + ',')

            results_file.write(re.search(r"^PWRUNDEAD dependencies per thread: ([(\d)| ]+)$", result_pwrundead.stdout, flags=re.MULTILINE).group(1) + ",")
            results_file.write(re.search(r"^PWRUNDEAD dependencies per thread: ([(\d)| ]+)$", result_pwrundeadguard.stdout, flags=re.MULTILINE).group(1) + ",")

            results_file.write(re.search(r"^Possible guard lock dependencies: (\d+)$", result_pwrundeadguard.stdout, flags=re.MULTILINE).group(1) + ",")
            results_file.write(re.search(r"^Possible guard locks: (\d+)$", result_pwrundeadguard.stdout, flags=re.MULTILINE).group(1) + ",")
            results_file.write(re.search(r"^Guard locks accepted: (\d+)$", result_pwrundeadguard.stdout, flags=re.MULTILINE).group(1) + ",")
            results_file.write(re.search(r"^Guard locks declined: (\d+)$", result_pwrundeadguard.stdout, flags=re.MULTILINE).group(1) + "\n")
...
def create_graph_dependencies(results):
    logfiles = {}
    for result in results:
        if sum(results[result]['dependencies_per_thread_pwrundead']) == 0 or sum(results[result]['dependencies_per_thread_pwrundead']) == sum(results[result]['dependencies_per_thread_pwrundeadguard']):
            continue
...
def create_deps_table(results):
    with open(os.path.join(RESULTS_PATH, 'deps_table.csv'), 'w') as results_file:
        results_file.write('trace,possible_guard_lock_dependencies,possible_guard_locks,guard_locks_accepted,guard_locks_declined\n')
